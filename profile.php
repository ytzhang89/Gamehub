...
<textarea id="post" name="post" rows="3" cols="60"></textarea>
...
if (in_array($user,$friendArray)) {
 $addAsFriend = '<input class = "btn btn-small btn-info" type="submit" name="removefriend" value="UnFriend">';
}
else
{
 $addAsFriend = '<input class = "btn btn-small btn-info" type="submit" name="addfriend" value="Add Friend">';
}
echo $addAsFriend;
}
else
...
{
  $date = date("Y-m-d");
  $create_button = mysql_query("INSERT INTO like_buttons VALUES ('', '$username', 'http://localhost/database/$username', '$date', '$username')");
  $insert_like = mysql_query("INSERT INTO likes VALUES('','$username','-1','$username')");
}
?>
<?php echo $errorMsg; ?>
<input onclick="window.open('send_msg.php?u=<?php echo $username ?>','_self')" class = "btn btn-small btn-info" type="button" name="sendmsg" value="Message" />
<iframe src='http://localhost/database/like_but_frame.php?uid=<?php echo $username; ?>' style='border: 0px;height: 23px; width: 110px;'> </iframe>
